...
// === Wander to target function ===
function wanderToTarget(targetPos) {
...
function wanderLoop(targetPos) {
  if (!wanderRunning) return;
  wanderToTarget(targetPos);
  requestAnimationFrame(() => wanderLoop(targetPos));
}

function startWandering(targetPos) {
  if (wanderRunning) return;
  wanderRunning = true;
  startMeleeAttack();
  requestAnimationFrame(() => wanderLoop(targetPos));
...
function toggleWandering(targetPos) {
  if (wanderRunning) {
    stopWandering();
  } else {
    startWandering(targetPos);
  }
}
